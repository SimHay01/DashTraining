...
id=utils.id_factory('dashboard_func_next_next_year')
...
def get_Layout():
    ...
    columnDefs,defaultColDef,getRowStyle=dataframe.get_dashboard_functional(next_next_year)
    layout=html.Div(
        [
            ...
            dag.AgGrid(
                id=id("dashboard-functional-next-next-year"),
                columnDefs=columnDefs,
                rowData=dataframe.get_dataframe_functional(dataframe.dashboard_func,dataframe.simu_func,next_next_year,year,next_year).to_dict("records"),
...
@app.callback(Output(id('dashboard-functional-next-next-year'),'filterModel',allow_duplicate=True),
...
@app.callback(
    Output(id("dashboard-functional-next-next-year"),"dashGridOptions",allow_duplicate=True),
    Input(id("dashboard-functional-next-next-year"), "selectedRows"),
)
