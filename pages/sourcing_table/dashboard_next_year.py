...
import dash
from dash import html, Patch, clientside_callback, ClientsideFunction
from app import app
...
id=utils.id_factory('dashboard_next_year')
...
def get_Layout():
    ...
    data=dataframe.get_dataframe(dataframe.dashboard_ope,dataframe.simu_ope,next_year,year,next_next_year)
...
@app.callback(Output(id('dashboard-next-year'),'filterModel',allow_duplicate=True),
...
@app.callback(
    Output(id("dashboard-next-year"), "columnState"),
    Input(id("columns_to_hide"), "value"),
    State(id("dashboard-next-year"), "columnState"),
    prevent_initial_call=True,
)
    ...
    for service in selected_services:
        ...
        for timeframe in selected_timeframe:
            if timeframe=="full_year":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the year {dataframe.next_year}):",style={"marginTop":"20px"}),
                    dbc.Row([
            ...
            if timeframe=="first_half":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the 1st half of {dataframe.next_year}):",style={"marginTop":"20px"}),
                    dbc.Row([
                        dbc.Col([html.H4("Workload :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workload} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P("a",style={"fontSize":"large","opacity":0}), html.P("a",style={"fontSize":"large","opacity":0})],style={"textAlign":"center"}),
                        dbc.Col([html.H4("Workforce :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workforce} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P([f"- Headcounts:",html.B(f" {hc} FTEs")],style={"fontSize":"large"}), html.P([f"- Contractors:",html.B(f" {cont} FTEs")],style={"fontSize":"large"})],style={"textAlign":"center"}),
                        dbc.Col(dcc.Graph(figure=fig_cover)),
            ...
            if timeframe=="second_half":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the 2nd half of {dataframe.next_year}):",style={"marginTop":"20px"}),
                    dbc.Row([
                        dbc.Col([html.H4("Workload :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workload} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P("a",style={"fontSize":"large","opacity":0}), html.P("a",style={"fontSize":"large","opacity":0})],style={"textAlign":"center"}),
                        dbc.Col([html.H4("Workforce :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workforce} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P([f"- Headcounts:",html.B(f" {hc} FTEs")],style={"fontSize":"large"}), html.P([f"- Contractors:",html.B(f" {cont} FTEs")],style={"fontSize":"large"})],style={"textAlign":"center"}),
                        dbc.Col(dcc.Graph(figure=fig_cover)),
...
@app.callback(
    Output(id("dashboard-next-year"),"dashGridOptions",allow_duplicate=True),
    Input(id("dashboard-next-year"), "selectedRows"),
)
