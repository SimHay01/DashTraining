...
id=utils.id_factory('dashboard_func_next_year')
...
@app.callback(Output(id('dashboard-functional-next-year'),'filterModel',allow_duplicate=True),
...
def get_Layout():
    ...
    layout=html.Div(
        [
            ...
            dag.AgGrid(
                id=id("dashboard-functional-next-year"),
                columnDefs=columnDefs,
                rowData=dataframe.get_dataframe_functional(dataframe.dashboard_func,dataframe.simu_func,next_year,year,next_next_year).to_dict("records"),
...
@app.callback(
    Output(id("dashboard-functional-next-year"),"dashGridOptions",allow_duplicate=True),
    Input(id("dashboard-functional-next-year"), "selectedRows"),
)
