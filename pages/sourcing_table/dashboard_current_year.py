...
Q1_HC_TOPS=utils.Q1_HC_TOPS
Q2_HC_TOPS=utils.Q2_HC_TOPS
Q3_HC_TOPS=utils.Q3_HC_TOPS
Q4_HC_TOPS=utils.Q4_HC_TOPS
H1_HC_TOPS=utils.H1_HC_TOPS
H2_HC_TOPS=utils.H2_HC_TOPS
AVG_HC_TOPS=utils.AVG_HC_TOPS
...
# Generates a unique ID to the dashboard-current-year page
id=utils.id_factory('dashboard-current-year')
#=====================================================================================
...
# We use a function rather than a variable to dynamically update the data from the dataframe
def get_Layout(login_status):
    year=dataframe.year
    next_year=dataframe.next_year
    next_next_year=dataframe.next_next_year
    columnDefs,defaultColDef,getRowStyle=dataframe.get_dashboard(year)
    data=dataframe.get_dataframe(dataframe.dashboard_ope,dataframe.simu_ope,year,next_year,next_next_year)
    ...
    if login_status == 'authenticated':
        ...
        [
            html.H2(f'Dashboard {year}',style={"textAlign":"center","color":"#23004C", "paddingTop":"10px"}),
            html.Hr(),
            html.Div([
                html.Div(dbc.Button('Collapsed view',id=id('collapsed_button'),n_clicks=0,class_name="me-2", style={"backgroundColor":"#7A00E6", "borderColor": "#7A00E6"}),style={'width': '10%', 'display': 'inline-block'}),
                    ...
                    dbc.ModalBody([
                        html.Div(
                            [
                                ...
                                dbc.Checklist(
                                    options=[
                                        {"label": f"Average data for the year {year}", "value": "full_year"},
                                        {"label": f"1st half of {year}", "value": "first_half"},
                                        {"label": f"2nd half of {year}", "value": "second_half"},                                    
                                    ],
                                    value=["full_year"],
        ...
        layout=html.Div([
            html.H1('Access to this page is restricted'),
            html.P("Access to this page is restricted to those responsible for managing baselines and TOPs. If you wish to view the data of a completed baseline, please go to the “Archives” page of the application."),
            html.P('Please enter the password to access this page'),
            dcc.Input(id='password-input', type='password', placeholder='Password'),
            html.P(' '),
            dbc.Button('Submit', id='submit-password', class_name="me-2", style={"backgroundColor":"#7A00E6", "borderColor": "#7A00E6"}),
...
@app.callback(
    ...
    Output(component_id='login-status', component_property='data'), # authentification status of the user
    Output(component_id='login-output', component_property='children'), # message displayed below the Submit button after clicking on it
    Input(component_id='submit-password', component_property='n_clicks'), # Submit button
    State(component_id='password-input', component_property='value'), # password value entered by the user
    State(component_id='login-status', component_property='data')
)
def verify_password(n_clicks, password, login_status):
    """Verify the password entered by the user after clicking on the submit button.
    Return the authentifaction status of the user & a message.
    """
    if n_clicks and password == PASSWORD:
        return 'authenticated', 'Correct password. Access granted!'
    elif n_clicks:
        return dash.no_update, 'Incorrect password, please try again.'
    return login_status, ''

@app.callback(
        Output(component_id='dashboard-current-year', # AG grid of the page
               component_property='filterModel', # State of filter(s) applied to the AG grid
               allow_duplicate=True), # allows multiple callbacks to update the AG grid
        Input(component_id=id('collapsed_button'), # 'Collapsed view' button
              component_property='n_clicks'), 
        prevent_initial_call=True)

def collapsed_view(n_clicks):
    """Applies "ALL" filter to the “Role” column when the "Collapsed view" button is pressed.

    Args:
        n_clicks (int): number of click, indicates whether the button has been pressed or not

    Returns:
        filterModel: apply a filter to the Ag Grid
    """
...
@app.callback(Output('dashboard-current-year','filterModel',allow_duplicate=True),
              Input(id('corpo_button'),'n_clicks'), # 'Corporate view' button
              prevent_initial_call=True)
def corporate_view(n_clicks):
    """
    Applies a filter to the Ag Grid and keeps only data corresponding to services belonging
    to “Corporate Centers” when the "Corporate view" button is pressed.
...
@app.callback(Output('dashboard-current-year','filterModel',allow_duplicate=True),
              Input(id('csu_button'),'n_clicks'), # 'CSU view' button
              prevent_initial_call=True)
def csu_view(n_clicks):
    """
    Applies a filter to the Ag Grid and keeps only data corresponding to services belonging 
    to “CSUs” when the "CSUs view" button is pressed.
...
@app.callback(Output('dashboard-current-year','filterModel',allow_duplicate=True),
              Input(id('reset_button'),'n_clicks'), # 'Reset view' button
              prevent_initial_call=True)
def reset_view(n_clicks):
    """
    Reset any filter applied to the Ag Grid when the "Reset view" button is pressed.
...
@app.callback(
    Output('dashboard-current-year', "columnState"), # state of grid's columns (order, visibility, width, sort)
    Input(id("columns_to_hide"), "value"), # Under the 'Hide columns' button, list of columns the user wishes to hide
    State('dashboard-current-year', "columnState"),
    prevent_initial_call=True,
)
def hide_columns(value,state):
    """
    Hides selected columns

    Args:
        value : User-selected columns in checklist to hide
        state : Columns current state

    Returns:
        columnState: New columns state
    """
    col_to_hide=[]
    ...
    for col in state:
        # 'hide' is a property of an AG column
        # 'colId' is a unique ID assigned to each column of the AG grid
        # set hide to True if column's ID is found in the col_to_hide list ; otherwise False
        col['hide'] = (True and col['colId'] in col_to_hide)        
    
    return state

@app.callback(
    Output(id("modal"), "is_open"), # Pop-up displaying graphic view of the indicators
    Input(id("graphic_button"), "n_clicks"), # 'Graphic view' button
    Input(id("close"), "n_clicks"), # 'Close button' of the pop-up
    State(id("modal"), "is_open"), # opening status of the pop-up
)
def toggle_modal(n1, n2, is_open):
    """Hide or display the pop-up displaying graphic view of the indicators
    Return the status of the pop-up
    """
    # by default, the pop-up status is set to closed, so the if the user clicks
    # on the 'Graphic view' button (n1), it will open it
    # on the 'Close' button of the pop-up (n2), it will close it
    if n1 or n2:
        return not is_open
    return is_open
...
def update_modal_content(selected_services,selected_timeframe):
    """
    Update the content of the "Graphic view" modal based on the selection of the user. 
    Displays graphics to show the data corresponding to the service and to the timeframe
    selected by the user

    Args:
        selected_services (list): Services selected by the user
        selected_timeframe (list): Timeframes selected by the user
    ...
    if selected_services is None:
        return content    
    elif selected_timeframe is None:
        return content
    ...
    for service in selected_services:
        ...
        for timeframe in selected_timeframe:
            if timeframe=="full_year":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the year {dataframe.year}):",style={"marginTop":"20px"}),
                    dbc.Row([
            ...
            if timeframe=="first_half":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the 1st half of {dataframe.year}):",style={"marginTop":"20px"}),
                    dbc.Row([
                        dbc.Col([html.H4("Workload :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workload} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P("a",style={"fontSize":"large","opacity":0}), html.P("a",style={"fontSize":"large","opacity":0})],style={"textAlign":"center"}),
                        dbc.Col([html.H4("Workforce :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workforce} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P([f"- Headcounts:",html.B(f" {hc} FTEs")],style={"fontSize":"large"}), html.P([f"- Contractors:",html.B(f" {cont} FTEs")],style={"fontSize":"large"})],style={"textAlign":"center"}),
                        dbc.Col(dcc.Graph(figure=fig_cover)),
            ...
            if timeframe=="second_half":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the 2nd half of {dataframe.year}):",style={"marginTop":"20px"}),
                    dbc.Row([
                        dbc.Col([html.H4("Workload :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workload} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P("a",style={"fontSize":"large","opacity":0}), html.P("a",style={"fontSize":"large","opacity":0})],style={"textAlign":"center"}),
                        dbc.Col([html.H4("Workforce :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workforce} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P([f"- Headcounts:",html.B(f" {hc} FTEs")],style={"fontSize":"large"}), html.P([f"- Contractors:",html.B(f" {cont} FTEs")],style={"fontSize":"large"})],style={"textAlign":"center"}),
                        dbc.Col(dcc.Graph(figure=fig_cover)),
...
@app.callback(
    Output('dashboard-current-year',"dashGridOptions",allow_duplicate=True),
    Input('dashboard-current-year', "selectedRows"),
)
def row_pinning_bottom(selection):
    """
    Displays a pinned row at the bottom of the grid when the user selects one or more lines. 
    This pinned row will display the aggregated data of the different selected lines.

    Args:
        selection (selectedRows): Rows selected by the user in the Ag-Grid

    Returns:
        dashGridOptions: Options of the Ag-Grid, determines the presence or not of a pinned row and the content of it. 
    """
