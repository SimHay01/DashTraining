...
id=utils.id_factory('archive-dashboard-current-year')
#=====================================================================================
...
def get_Layout(year,data):
    ...
    if not isinstance(year,int):
        ...
        [
            ...
            dag.AgGrid(
                id="archive-dashboard-current-year",
...
@app.callback(Output('archive-dashboard-current-year','filterModel',allow_duplicate=True),
              Input(id('global_button'),'n_clicks'),
              prevent_initial_call=True)
def global_view(n_clicks):
    """
    Applies a filter to the “Role” column with the value “ALL" when the "Global view" button is pressed.
...
def corporate_view(n_clicks):
    """
    Applies a filter to the Ag Grid and keeps only data corresponding to services belonging to “Corporate Centers” when the "Corporate view" button is pressed.
...
def csu_view(n_clicks):
    """
    Applies a filter to the Ag Grid and keeps only data corresponding to services belonging to “CSUs” when the "CSUs view" button is pressed.
...
@app.callback(
    Output('archive-dashboard-current-year', "columnState"),
    Input(id("columns_to_hide"), "value"),
    State('archive-dashboard-current-year', "columnState"),
    prevent_initial_call=True,
)
...
@app.callback(
    ...
    State("data_ope_year","data"),
    prevent_initial_call=True,
)
def update_modal_content(selected_services,selected_timeframe,df):
    """
    Update the content of the "Graphic view" modal based on the selection of the user. Displays graphics to show the data corresponding to the service and to the timeframe
    ...
    for service in selected_services:
        ...
        for timeframe in selected_timeframe:
            if timeframe=="full_year":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the year {archive.year}):",style={"marginTop":"20px"}),
                    dbc.Row([
            ...
            if timeframe=="first_half":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the 1st half of {archive.year}):",style={"marginTop":"20px"}),
                    dbc.Row([
                        dbc.Col([html.H4("Workload :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workload} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P("a",style={"fontSize":"large","opacity":0}), html.P("a",style={"fontSize":"large","opacity":0})],style={"textAlign":"center"}),
                        dbc.Col([html.H4("Workforce :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workforce} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P([f"- Headcounts:",html.B(f" {hc} FTEs")],style={"fontSize":"large"}), html.P([f"- Contractors:",html.B(f" {cont} FTEs")],style={"fontSize":"large"})],style={"textAlign":"center"}),
                        dbc.Col(dcc.Graph(figure=fig_cover)),
            ...
            if timeframe=="second_half":
                ...
                row= html.Div([
                    html.H3(f"{service}/{code} (average data for the 2nd half of {archive.year}):",style={"marginTop":"20px"}),
                    dbc.Row([
                        dbc.Col([html.H4("Workload :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workload} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P("a",style={"fontSize":"large","opacity":0}), html.P("a",style={"fontSize":"large","opacity":0})],style={"textAlign":"center"}),
                        dbc.Col([html.H4("Workforce :",style={"fontWeight":"normal","fontSize":"xx-large"}), html.P(f"{workforce} FTEs",style={"fontWeight":"bold","fontSize":"x-large"}), html.P([f"- Headcounts:",html.B(f" {hc} FTEs")],style={"fontSize":"large"}), html.P([f"- Contractors:",html.B(f" {cont} FTEs")],style={"fontSize":"large"})],style={"textAlign":"center"}),
                        dbc.Col(dcc.Graph(figure=fig_cover)),
...
@app.callback(
    Output('archive-dashboard-current-year',"dashGridOptions",allow_duplicate=True),
    Input('archive-dashboard-current-year', "selectedRows"),
)
def row_pinning_bottom(selection):
    """
    Displays a pinned row at the bottom of the grid when the user selects one or more lines. This pinned row will display the aggregated data of the different selected lines.
